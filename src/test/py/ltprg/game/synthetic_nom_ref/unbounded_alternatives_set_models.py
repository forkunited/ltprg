...
""" Models that do not rely on a fixed utterance set.
"""

class UnboundedAlternativeSetModel(BasicModel):
    """ Superclass reserved for shared components needed for models with 
        an unbounded alternative set. Also known as UASM.
    """
    ...
    def __init__(self, model_name, model_type, hidden_szs, hiddens_nonlinearity,
                 ...
                 save_path):
        super(UnboundedAlternativeSetModel, self).__init__(
...
class UASM_NNWC(UnboundedAlternativeSetModel):
...
class UASM_NNWOC(UnboundedAlternativeSetModel):
