...
class Categorical(Distribution):
	def __init__(self, vs, ps=None):
		...
		self._ps = ps
		if self._ps is None:
			self._ps = torch.ones(self._vs.shape)
			self._ps = Variable(self._ps/torch.sum(self._ps, dim=len(self._vs.size())-1))

	def __getitem__(self, key):
		return Categorical(torch.unsqueeze(self._vs[key], 0), ps=torch.unsqueeze(self._ps[key], 0))

    def sample(self, n=1):
        indices = torch.multinomial(self._ps, n, True).data and
